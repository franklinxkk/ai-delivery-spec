...
- 无列表入口权限：菜单与列表入口不可见；直接访问旧地址沿用既有拒绝结果，不能通过请求参数绕过权限。
...
| AC/TEST | 前提（角色与数据） | 操作 | 可见结果 | 领域结果/反例证据 |
|---|---|---|---|---|
...
| AC-POLICY-ACTIVE-003 / TEST-POLICY-ACTIVE-003 | 用户无制度列表入口权限 | 查看导航、直接访问旧地址或构造筛选请求 | 导航中没有制度列表入口；直接地址显示既有拒绝结果 | 服务端拒绝请求，不能因筛选参数扩大数据范围 |
