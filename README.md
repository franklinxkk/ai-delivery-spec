...
[![Version](https://img.shields.io/badge/version-5.2.0-0052A4.svg)]()
...
**公开采用信号（截至 2026-07-16）**：
[ClawHub 732 次下载](https://clawhub.ai/franklinxkk/skills/ai-delivery-spec) ·
[skills.sh 16 次安装](https://www.skills.sh/franklinxkk/ai-delivery-spec) ·
[SkillHub TRACE 4.5/5](https://skillhub.cn/skills/ai-delivery-spec)，双安全扫描无 P0/P1 风险。
...
集中 FAQ、常见错误与反模式见[排障与恢复](references/runtime/troubleshooting.md)。
...
7. **可恢复而不自动改需求**：每个门禁发现给出原因、修复方向和原命令重试入口；
   大项目从经过校验的阶段/ID 检查点继续。
8. **数据领域进入生产与流通链路**：覆盖登记、授权运营、可信数据空间、数据产品、
   高质量数据集、标注、训练/评测/偏好数据、模型反馈与价值验证，不再止于报表问数。
9. **双层质量保障**：发布前用跨行业多 Agent 组合找方法缺口；每个项目末端只运行
...
`handoff`、`full` 五个按需 Profile；`handoff` 交叉检查一份 PRD 与多个后台/H5 原型。
默认最多报告 20 个问题。静态通过不替代领域负责人、浏览器
关键旅程或客户验收。
...
| **需求管理内核** | **AI Delivery Spec 5.2.0** | 准入 → 澄清 → 基线 → 变更 → 追溯 → 验收 |
...
仓库同时受 190 文件预算门禁约束，防止测试、证据和适配说明继续无序扩散。
...
python maintainer/tests/test_v511_role_stage.py
