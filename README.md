...
它的目标不是机械地让 PRD 变厚，而是让产品全生命周期的单一交付物或完整交付包都能被准确触发、独立评审、正确路由并在明确边界停止。v4.3.0 以传统产品需求规格为主文档，通过逐功能 FRR、工程追溯契约、权威证据覆盖和 Reader-first 导航，避免功能模块被压缩成无法开发和测试的摘要，同时降低产研团队阅读成本。
...
v4.0-v4.1.0 已解决分级交付、防御性边界、模板、战略交接、精准触发、范围一致性、场景回归、全球化准备和全生命周期评审。v4.2.1 修复复杂 PRD 容易“模块有摘要，但功能仍无法直接开发”的问题。v4.3.0 进一步把 Reader-first 可读性作为增强层加入，但不替代完整 FRR 和工程契约：

- Gate 3 同时验收产品规格完整性和工程追溯契约，两者不可互相替代；
- Excel/PDF/SQL/原型/截图/规则目录进入 Source Evidence Register，要求零静默遗漏；
- 传统 PRD 恢复为主产品事实，工程契约只能补充、不能替代；
- 每个模块先建立完整功能清单，再为每个首发功能编写 15 段确定性 FRR；
- `FULL_SPEC` 改为计算结果：首发功能数必须等于完整 FRR 数；
- 新增 Reader Navigation，让产品、前端、后端、算法、测试、架构在同一份 PRD 内按角色快速定位；
- 新增 Source Assertion Status，区分 `VERIFIED / INFERRED / PROPOSED / UNKNOWN / CONFLICT`，避免建议被静默写成事实；
- 新增 Prototype Evidence Status，区分 `VERIFIED / SPEC_ONLY / GAP / CONFLICT / UNKNOWN`，允许 Stage 0 抽取通过但 Gate 2/3 继续阻断；
- 强化语义拆分规则：角色、权限、触发器、数据所有者、状态迁移、业务结果、审计/NFR 或验收路径不同就要拆成功能；
- 强化 AI 报告/AI-core 合同：版本化 Schema、逐 claim 证据、Prompt/模型/检索/运行版本、评测集、降级和禁写边界；
- Dashboard、指标库、Excel 报表、企业填报、AI 数据报告获得专项详细模板；
- 复杂度页数预算只约束主叙事或单模块，超量时拆分，不再删减字段、规则和验收标准。
