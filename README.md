...
它的目标不是让 PRD 变厚，而是让产品交付物真正可开发、可演示、可测试、可治理。v4.0.2 采用分级交付模型和条件 Gate：先判断项目复杂度，再按需加载移动端、多端一致性、ToB/ToG 审批、多租户/RBAC、报表指标、低代码工作流、AI 嵌入、AI Native Harness、Prompt Ops、效果评估等协议，并补充生产落地所需的模板和防御性契约。
...
v4.0 解决了早期版本“全量协议过重”的问题。v4.0.1 补齐企业真实交付中的防御性边界。v4.0.2 进一步降低产品团队采用成本，补充开源许可证、PRD 模板、低成本 Prompt Ops、跨端 Gate 2 验收和上线就绪检查。
