# AI Delivery Spec 5.3.0 — Requirements Humans Can Read, Coding Agents Can Execute
...
[![Version](https://img.shields.io/badge/version-5.3.0-0052A4.svg)]()
...
<!-- CLAIM: CLM-ADOPTION-20260718; as_of=2026-07-18; evidence=public-platform-links-below -->
**公开采用信号 / Public Adoption Signals（截至 2026-07-18）**：
[ClawHub 792 次下载](https://clawhub.ai/franklinxkk/skills/ai-delivery-spec) · [skills.sh 22 次安装](https://www.skills.sh/franklinxkk/ai-delivery-spec) ·
...
- **Gate｜门禁**：零模型静态检查结构、锚点和追溯；PASS 不代表业务正确、浏览器已跑或客户已签字。
...
首选修正：`delivery_shape=requirement_card|unified_prd|governed_truth assurance_profile=bounded|standard|high_risk|safety_critical domain=<pack>`；旧 `mode/tier` 仍兼容。
...
- **Prototype When Needed｜按需原型**：存量先做 Stage 0；新原型的动作必须有处理器和可见结果。
...
## 5.3.0 — Stronger Contract, Lighter Runtime

结构完整不等于可读、可施工。5.3.0 聚焦五个真实失败：不可读的机器主文档、两套 PRD、
默认巨型 Truth、未盘点就覆盖存量原型，以及多 Agent 切片后的基线/横切规则漂移。
...
python scripts/ai_delivery_spec_cli.py gate --profile stage0 --inventory requirements/stage0.yaml
python scripts/ai_delivery_spec_cli.py gate --profile agent_handoff --manifest requirements/handoff-manifest.yaml
python scripts/query_domain.py --domain data-product --section "Core Workflows" --format markdown
python scripts/ai_delivery_spec_cli.py candidate validate --input requirements/domain-candidate.yaml
python scripts/ai_delivery_spec_cli.py explain-finding PRD-STRUCTURE
python scripts/ai_delivery_spec_cli.py resume
python scripts/ai_delivery_spec_cli.py init-custom
python scripts/ai_delivery_spec_cli.py init-requirements --output requirements --custom-root custom --template my-team
python scripts/ai_delivery_spec_cli.py query-domain --domain traffic+my-team --custom-root custom --format yaml
```

`gate` 只判定和定位，不生成、不自动修复；支持 `requirement`、`prd`、`prototype`、
`handoff`、`full`、`stage0`、`agent_handoff` 七个按需 Profile；`handoff` 交叉检查一份 PRD 与多个后台/H5 原型。
默认只显示第一个问题，`--diagnostics summary` 最多显示 12 个。静态通过不替代领域负责人、浏览器
关键旅程或客户验收。

`custom/validators/*.yaml` 只支持 `must_match/must_not_match` 声明式正则，Skill 不执行项目本地
Python 校验器，避免私有扩展变成任意代码入口。官方规则先运行；私有模板/展示默认可覆盖，法规、
权限、资金、安全等绑定规则冲突必须由责任人登记 `DEC-CONFLICT-*`。
...
| **需求管理内核** | **AI Delivery Spec 5.3.0** | 准入 → 澄清 → 基线 → 变更 → 追溯 → 验收 |
