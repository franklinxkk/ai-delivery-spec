...
generic_dispatch_html = """<!doctype html><html><body>
<main data-testid="page-VIEW-X" data-state="ready">
  <button data-action="ACT-X-SAVE" data-ac="AC-X-001">Save</button>
</main>
<script>
...
with tempfile.TemporaryDirectory() as tmp:
    artifact = Path(tmp) / "generic-dispatch.html"
    artifact.write_text(generic_dispatch_html, encoding="utf-8")
    generic_dispatch = run("--profile", "prototype", "--prototype", str(artifact), "--level", "L3")
generic_codes = {item["code"] for item in json.loads(generic_dispatch.stdout)["findings"]}
if "PROTO-UNHANDLED-ACTION" not in generic_codes:
    failures.append("generic dispatcher without an explicit ACT mapping falsely proved action ownership")
...
baseline_expected = {
    "PRD-NO-SOURCE-SCOPE", "PRD-NO-SOURCE-REGISTER",
    "PRD-NO-P0-DISPOSITION", "PRD-AC-NO-MACHINE-DEFINITION",
}
if not baseline_expected.issubset(walked_codes):
    failures.append(f"L3 source/P0/machine-AC baseline gaps escaped: {sorted(walked_codes)}")
