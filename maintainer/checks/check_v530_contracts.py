...
open_p0_unknown_ids: []
---
# 列表字段调整需求卡 REQ-LIGHT-001
## 0. 来源、问题与价值
REQ-LIGHT-001 来源于已确认客服反馈；准入结论为接受。运营跨页查询归属地耗时，本次目标是减少查询切换。
## 1. 目标、范围与非目标
范围只增加只读归属地；不改变编辑、权限、筛选逻辑和存量数据。
## 2. 角色、用户故事与权限
ROLE-OPERATOR（运营）希望在列表查看归属地；仅展示其原有数据范围内的记录，不扩大本部门权限。
## 3. 用户旅程、流程、异常与状态 FLOW-LIGHT-001
FLOW-LIGHT-001 从 ROLE-OPERATOR 进入列表开始；字段随列表加载，数据缺失显示短横线，加载失败按原页面恢复机制处理。
## 4. 规则、字段与条件规格
归属地为只读展示字段，沿用记录权威源；空值显示短横线，不新增录入、编辑、排序或状态变化。
## 5. 验收与测试 AC-LIGHT-001
AC-LIGHT-001 要求有权用户打开列表后显示归属地；空值显示短横线；越权用户不得获得额外数据。
## 6. 未知项与升级判断
当前无开放未知项；若新增筛选、编辑、历史补数或跨部门可见性，则升级范围并重新评审。
""", encoding="utf-8")
