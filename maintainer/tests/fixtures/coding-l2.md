...
业务确认目标与范围；前端、后端和测试先读 MOD-ITEM 纵向切片，再按稳定 ID 查附录；
Coding Agent 只实现指定 REQ/MOD，不跳过正文猜规则。

### 0.2 30 秒摘要

ROLE-ADMIN 从 VIEW-HOME 登记事项；系统校验权限、字段和版本后创建唯一 active 事项，
显示编号并可在列表检索。失败保留输入并允许恢复；AC-ITEM-001 以界面、接口和审计证据验收。
...
模块目标是让 ROLE-ADMIN 从 VIEW-HOME 入口完成事项创建；边界是不含批量导入和外部同步，
前置为已登录当前租户，成功结果是 active 事项可见。用户故事和主路径见 FLOW-ITEM-CREATE；
异常包含无权、字段失败和版本冲突，恢复方式为保留输入后重试。页面动作使用 ACT-SUBMIT；
FLD-NAME 的数据来源为用户输入，权限限定当前租户。RULE-NAME-001 约束字段，STATE-ITEM
由 EVT-ITEM-CREATED 记录结果，外部集成不适用。指标口径按 clientRequestId 去重并按自然日刷新，
用于数据质量核对。验收为 AC-ITEM-001；无未决项。

#### 7.1.1 页面与布局 VIEW-HOME

页面顶部显示标题，中部 REG-FORM 含名称字段和提交按钮，下部显示最近创建结果。
加载、空、错误、无权限和成功状态均必须有明确文字，不用颜色单独表达。

#### 7.1.2 关键操作 ACT-SUBMIT
...
涉及高影响或不可逆结果时必须由已授权责任人完成人工确认，并保留确认人和证据。
