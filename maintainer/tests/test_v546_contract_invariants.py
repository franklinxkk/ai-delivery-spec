...
def custom_gate(root: Path, *domains: str) -> dict:
    ...
    dump(custom / 'validators/ev.yaml', {'rules': [{'id': 'CUST-EV-DRIVER-SAFETY', 'artifact': 'prd', 'assertion': 'must_match', 'domains': ['electric-vehicle'], 'severity': 'BLOCK', 'pattern': '驾驶安全'}]})
    prd = root / 'prd.md'
    prd.write_text('---\ndocument_language: zh-CN\ndelivery_level: L0\n---\n# 证券需求\n## 范围\n自选证券。\n## 验收\n保存后可见。\n', encoding='utf-8')
    command = ['scripts/ai_delivery_spec_cli.py', 'gate', '--profile', 'prd', '--prd', str(prd), '--level', 'L0', '--custom-root', str(custom), '--format', 'json', '--diagnostics', 'full']
...
def test_prd_checks_conflicts_and_duplicate_definitions_across_tables() -> None:
    ...
    clean = body.replace('| UNK-AUDIT-101 | maximum upload size | open |', '| UNK-AUDIT-101 | retention period | open |').replace('| REQ-AUDIT-101 | validate after submit |', '| REQ-AUDIT-102 | validate after submit |')
...
def prototype_findings(path: Path, iframe: str, level: str) -> list:
    path.write_text(f'<main data-testid="page-VIEW-AUDIT-001"><section data-testid="region-REG-AUDIT-001">{iframe}</section></main>', encoding='utf-8')
    gate = Gate()
...
def test_remote_iframe_has_level_aware_trust_contract(tmp_path: Path) -> None:
    ...
    declared = prototype_findings(tmp_path / 'declared.html', '<iframe src="https://example.com" data-integration-ref="INT-EXT-001" data-fallback="显示重试" title="外部系统" sandbox referrerpolicy="no-referrer"></iframe>', 'L2')
...
def test_metric_ids_cannot_collapse_distinct_business_meanings(tmp_path: Path) -> None:
    bad = _prototype_codes(tmp_path / 'bad-metric.html', '<main data-testid="page-VIEW-METRIC-001"><div data-metric="METRIC-DYNAMIC-CARD">Pending enterprises 8</div><div data-metric="METRIC-DYNAMIC-CARD">Abnormal vehicles 12</div></main>')
    ...
    good = _prototype_codes(tmp_path / 'good-metric.html', '<main data-testid="page-VIEW-METRIC-002"><div data-metric="METRIC-ENTERPRISE-PENDING" data-metric-label="Pending enterprises">Pending enterprises 8</div><div data-metric="METRIC-ENTERPRISE-PENDING" data-metric-label="Pending enterprises">Pending enterprises 12</div></main>')
...
def test_review_unknowns_require_owned_stage_aware_contracts(tmp_path: Path) -> None:
    incomplete = _prototype_codes(tmp_path / 'unknown-incomplete.html', '<body class="review-mode"><main data-testid="page-VIEW-UNK-001"><div data-metric="METRIC-UNK-001" data-unk="UNK-METRIC-001">待确认</div><button data-review-id="REV-UNK-001" aria-current="true">1</button><aside data-review-id="REV-UNK-001" aria-current="true">指标说明</aside></main></body>')
    assert 'PROTO-UNKNOWN-CONTRACT-INCOMPLETE' in incomplete
    complete = _prototype_codes(tmp_path / 'unknown-complete.html', '<body class="review-mode"><main data-testid="page-VIEW-UNK-002"><div data-metric="METRIC-UNK-002" data-unk="UNK-METRIC-002" data-unk-priority="P1" data-unk-owner="产品负责人" data-unk-blocks-stage="baseline" data-unk-affected-refs="METRIC-UNK-002,AC-UNK-002" data-unk-fallback="显示—且不参与验收">待确认</div><button data-review-id="REV-UNK-002" aria-current="true">1</button><aside data-review-id="REV-UNK-002" aria-current="true">指标说明</aside></main></body>')
    assert 'PROTO-UNKNOWN-CONTRACT-INCOMPLETE' not in complete
    registry = _prototype_codes(tmp_path / 'unknown-registry.html', '<main data-testid="page-VIEW-UNK-003"></main><script>const unknowns=[{unk:\'UNK-METRIC-003\',priority:\'P0\',owner:\'业务负责人\',blocks_stage:\'review\',affected_refs:[\'METRIC-UNK-003\'],fallback:\'隐藏指标\'}];</script>')
