...
def validate(path: Path) -> int:
    ...
    if isinstance(document, dict) and document.get("layout") == "progressive_shards":
        sys.path.insert(0, str(ROOT / "scripts"))
        from compile_product_truth import compile_index  # noqa: PLC0415

        document, compile_failures = compile_index(path, validate_final=False)
        if compile_failures:
