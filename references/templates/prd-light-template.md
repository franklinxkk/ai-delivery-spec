---
artifact: requirement_card
baseline_version: 0.1-draft
delivery_level: L1
delivery_shape: requirement_card
document_language: zh-CN
language_source: user_request
bilingual: false
activated_facets: []
status: draft
---

# {需求名称}需求卡

> 默认跟随用户当前请求的主要语言。除非用户明确要求双语，否则标题、正文、表头、
> 澄清问题和验收用例统一使用 `document_language`；稳定 ID、代码、API/字段名和专名不翻译。
> 需求卡与统一 PRD 遵循同一 Human-First 分层口径：卡内各节即“正文面向人”，一事一处、
> ID 就近标注；升级为统一 PRD 时，字段字典、机器验收等机器投影移入工程附录，
> 不在正文平铺（见 `references/specify.md` “Human-First 编辑规则”）。

## 0. 文档控制与 30 秒摘要

| 项目 | 内容 |
|---|---|
| REQ ID / 版本 | `REQ-LIGHT-001` / `v0.1` |
| 负责人 / 确认人 | `{owner}` / `{approver}` |
| 准入结论 | 接受/澄清/延后/拒绝（`accept/clarify/defer/reject`） |
| 一句话结果 | `{谁在什么场景完成什么结果}` |
| 成功信号 | `{可观察、可验收的结果}` |

## 1. 来源、问题与价值

现状与证据：

用户/业务问题：

为什么现在做：

## 2. 目标、范围与非目标

- 目标：
- 本期范围：
- 明确不做：
- 约束与依赖：

## 3. 角色、用户故事与权限

| ROLE/REQ | 场景 | 用户故事（角色—动作—价值） | 数据/权限边界 |
|---|---|---|---|
| ROLE-PRIMARY / REQ-LIGHT-001 | | 作为…，我希望…，从而… | |

## 4. 用户旅程、主流程、异常与状态

```text
入口 → 动作 → 可见结果 → 领域结果 → 下一步/完成
               └→ 失败原因 → 保留内容/重试/退出
```

| FLOW/ACT | 前置与输入 | 可见结果 | 领域/状态结果 | 失败与恢复 | AC |
|---|---|---|---|---|---|
| FLOW-LIGHT-001 / ACT-LIGHT-001 | | | | | AC-LIGHT-001 |

## 5. 规则、字段与条件规格

先填写所有需求共有的规则、权限和异常；再只展开 `activated_facets` 中被触发的规格：

| 条件规格 | 触发时必须补齐 |
|---|---|
| 界面（`ui`） | 页面入口/布局、字段控件/必填/长度/校验、动作、空/错状态，以及入口/数据/动作/字段权限分层 |
| 状态流转（`stateful`） | 状态、允许转换、角色/守卫、非法路径、撤销/重试 |
| 数据上报（`data_submission`） | 来源/映射/校验、提交状态、重试/幂等、审计、口径/时效/对账 |
| 外部集成（`integration`） | API/事件语义、权限、请求/响应、业务错误、幂等/补偿 |
| 批量导入导出（`batch_io`） | 文件格式/大小/数量、预检、部分失败、导出范围/脱敏 |
| 高风险（`high_risk`） | 权威来源、责任人、人工门、禁止自动化、回退与证据 |

不适用项写“`不适用 + 业务原因`”，不得删除被触发项或用“按需/灵活/支持”代替规则。

## 6. 验收与测试

| AC/TEST | 前提（角色与数据） | 操作/输入 | 可见结果 | 领域结果 | 反例/证据 |
|---|---|---|---|---|---|
| AC-LIGHT-001 / TEST-LIGHT-001 | | | | | |

## 7. 未知项与升级判断

| UNK/REV | 问题 | 优先级/阻断阶段 | 负责人 | 建议/取舍 | 关闭证据 |
|---|---|---|---|---|---|

出现以下任一项时升级为统一 PRD：跨角色交接、跨模块/跨系统、关键状态机、数据上报/
统计口径、批量导入导出、审批审计、外部集成、资金/隐私/监管、不可逆写入、迁移或版本兼容。
升级是补足同一 `REQ-*` 的规格，不得另造互相冲突的第二份需求。
