...
> 需求卡与统一 PRD 遵循同一 Human-First 分层口径：卡内各节即“正文面向人”，一事一处、
> ID 就近标注；升级为统一 PRD 时，字段字典、机器验收等机器投影移入工程附录，
> 不在正文平铺（见 `references/specify.md` “Human-First 编辑规则”）。
...
| 准入结论 | 接受/澄清/延后/拒绝（`accept/clarify/defer/reject`） |
| 一句话结果 | `{谁在什么场景完成什么结果}` |
| 成功信号 | `{可观察、可验收的结果}` |
...
| 条件规格 | 触发时必须补齐 |
|---|---|
| 界面（`ui`） | 页面入口/布局、字段控件/必填/长度/校验、动作、空/错状态，以及入口/数据/动作/字段权限分层 |
| 状态流转（`stateful`） | 状态、允许转换、角色/守卫、非法路径、撤销/重试 |
| 数据上报（`data_submission`） | 立即升级 L2；补来源/映射/校验、提交状态、重试/幂等、审计、口径/时效/对账 |
| 外部集成（`integration`） | 立即升级 L2；补 API/事件语义、权限、请求/响应、业务错误、幂等/补偿 |
| 批量导入导出（`batch_io`） | 立即升级 L2；补文件格式/大小/数量、预检、部分失败、导出范围/脱敏 |
| 高风险（`high_risk`） | 立即升级 L2+；补权威来源、责任人、人工门、禁止自动化、回退与证据 |

不适用项写“`不适用 + 业务原因`”，不得删除被触发项或用“按需/灵活/支持”代替规则。

## 6. 验收与测试

| AC/TEST | 前提（角色与数据） | 操作/输入 | 可见结果 | 领域结果 | 反例/证据 |
|---|---|---|---|---|---|
