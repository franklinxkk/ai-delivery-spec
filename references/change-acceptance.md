...
评审会上要求“修改第 1—4 点、删除第 5 点、新增第 6 点”时，数字只负责定位当时看到的卡片，不能充当
永久业务身份。先建立 `CHG-*`，把每条意见绑定原 `RVP-*` 与其 `subject_ref`，澄清它改变的是文案、业务
行为、范围还是验收；会改变实现或验收的意见必须同时更新对应 `REQ/RULE/STATE/AC`。

- 修改：业务身份未变时保留原 `RVP-*`，同步右栏自然语言、语义覆盖项、PRD、AC 和测试；行为身份改变时新建
  稳定 ID，并在变更记录中声明替代关系。
- 删除：从新基线的 `review_contexts[].review_point_refs`、`review_points[]`、语义覆盖分母和 DOM marker/card
  中移除；旧身份只保留在历史基线和 `CHG-*` 中，不得靠重编号伪装成仍然存在。
- 新增：先关闭会改变实现/验收的 P0 未知，再创建新的业务 ID、`RVP-*`、语义覆盖项、marker/card/target、
  AC 与正反例。未经确认的 Candidate 不能直接晋级为正式标注。
- 重排：可见序号按新 Declaration 在每个 CurrentContext 内从 1 重新计算；稳定 ID 不随显示序号变化。
- 隔离：新基线使用新版本/hash。旧基线的评审记录、进度和分享定位不得静默灌入新基线；目标已删除或
  baseline 不匹配时必须明确提示失效或要求重新定位。

完成条件不是“HTML 看起来改了”，而是变更记录、PRD、当前 Declaration、语义分母、产品 DOM、验收与回归
使用同一新基线，且浏览器证明 marker/card/target 双向联动、被删项不可再选、新增项可定位。
