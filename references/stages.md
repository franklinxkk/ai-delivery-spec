...
阶段工作站用于管理复杂度，不应制造复杂度。一个来源明确、范围局部、可逆且不改变跨模块状态、外部数据权威、权限范围、指标口径、迁移兼容、法规/安全责任或高影响 AI 写回的改动，默认走快速小改：只检查触达面，直接交付目标，用一个短屏说明差异、边界、正反验收和未证明事项，不创建阶段文件、不显示 L0/内部 ID、不逐站过门禁。未给出的可逆表现只可继承已核实的存量惯例；没有读到基线时必须记为非阻断 GAP，不得自行冻结列位置、空值文案、默认排序等精确规则。
...
四个快捷入口只覆盖本次目标，不改变工作站和产物：`/ads` 使用 `entry_stage=auto,target_stage=auto`；`/dig` 直达 clarify 并按 `discover.md` 深度镜头关闭 P0；`/prd` 以 specify 为目标，阻断时暂入 clarify 后自动返回；`/proto` 以可实施产品原型为目标，按原型阶段 P0 和评审投影确认规则推进。它们是意图别名，不是 CLI 子命令要求。宿主若在消息到达模型前拦截未知斜杠命令，改用其显式 Skill 调用形式或自然语言同义表达；不得把四个裸别名描述成跨宿主原生注册。
...
## 7. 5.4.6—5.4.8 兼容与迁移矩阵

兼容读取不等于自动升级，也不得把旧工件静默宣称为 5.4.8 基线：

| 输入工件 | 5.4.8 行为 | 可宣称上限 | 迁移动作 |
|---|---|---|---|
| 5.4.6 及以前普通 PRD/产品态原型 | 按其已声明旧 Schema/层级检查；保留存量页面和业务动作 | 旧合同范围内可视检/校验 | 若要进入新评审交接，先补 Stage 0、当前 PRD 稳定 ID 与 5.4.8 评审合同 |
| 5.4.6 及以前旧式评审叠加 | prototype 单检返回 `PROTO-REVIEW-WORKSPACE-LEGACY` GAP；handoff/full 阻断 | 仅可视检，不得称可交接 | 迁移为 CurrentContext、三页签、ProductLocation、双向标号与语义覆盖账本 |
| 5.4.7 普通 PRD/产品态原型 | 保留既有合同并按适用门禁检查，不强制重写业务内容 | 原基线声明范围 | 只有受本次变更影响的切面补 5.4.8 合同 |
| `5.4.7-final` 评审 manifest | 兼容读取 RC1/RC2，不反向强制 RC3 新字段 | 5.4.7 评审合同范围 | 修改评审语义、页面或二级上下文时升级到 5.4.8/RC3 |
| 新生成的 5.4.8 评审工件 | 必须是 RC3，并声明页面语义分母、`surface_types` 与 `secondary_context_refs` | 仅在实际证据等级内声明 | 缺关键覆盖直接阻断，不允许降级伪装为旧版本 |

迁移不得改写产品事实：先冻结原文件/hash，将门禁发现登记为 GAP/变更影响，只补当前目标需要的新合同。无法证明旧版本、来源或基线 hash 时保持 `BLOCKED`，不得猜测版本。
