...
## 7. 5.4.6—5.4.9 兼容与迁移矩阵

兼容读取不等于自动升级，也不得把旧工件静默宣称为 5.4.9 基线：

| 输入工件 | 5.4.9 行为 | 可宣称上限 | 迁移动作 |
|---|---|---|---|
| 5.4.6 及以前普通 PRD/产品态原型 | 按其已声明旧 Schema/层级检查；保留存量页面和业务动作 | 旧合同范围内可视检/校验 | 若要进入新评审交接，先补 Stage 0、当前 PRD 稳定 ID 与 5.4.9/RC4 评审合同 |
...
| 5.4.7 普通 PRD/产品态原型 | 保留既有合同并按适用门禁检查，不强制重写业务内容 | 原基线声明范围 | 只有受本次变更影响的切面补 5.4.9/RC4 合同 |
| `5.4.7-final` 评审 manifest | 兼容读取 RC1/RC2，不反向强制 RC3/RC4 新字段 | 5.4.7 评审合同范围 | 修改评审语义、页面或二级上下文时升级到 5.4.9/RC4 |
| 5.4.8/RC3 评审工件 | 兼容读取，保留语义分母与 CurrentContext，不强制改写已批准产品行为 | 5.4.8 评审合同范围 | 本次修改人类评审投影时升级到 5.4.9/RC4，补自然语言层、技术追溯、图决定和正反例 |
| 新生成的 5.4.9 评审工件 | 必须是 RC4；复杂场景按条件显示流程/状态/数据图，简单 CRUD 不堆图 | 仅在实际证据等级内声明 | 稳定 ID/字段枚举泄露、复杂场景漏图或缺正反例直接阻断 |
