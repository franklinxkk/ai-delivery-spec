...
- 接受（`accept`）：价值和责任人明确，按推荐形态继续；
- 澄清（`clarify`）：需求可能成立，但关键未知项会改变范围或结果；
- 延后（`defer`）：需求有效，但价值较低、依赖未满足或归属后续迭代；
- 拒绝（`reject`）：无受支持结果、重复、越界，或风险与收益明显不匹配且无缓解方案。
...
提问前先执行自查闸：检查用户材料、仓库和精确领域章节。可从证据回答的
事实先登记为推断项（`inferred`）并给确认建议，只把用户偏好、禁止项、权威和冲突留给用户决策。
...
每个问题包含推荐答案、证据、取舍和受影响 ID。用户同意后才把
有界推断升级为已确认（`confirmed`）；不同意则更新 `REV/UNK/DEC`，不能静默改范围。

只有 P0/P1 分支允许持续追问。P2及以下使用明确推荐默认或有责任人的
`UNK-*`。用户暂不可用时，记录假设、责任人、阻断阶段（`blocks_stage`）和回退路径，
不得静默替用户选择。
...
默认只建立一份统一 PRD；传统 PRD 和 AI Coding PRD 不得成为两个独立事实源。文档语言继承任务
起始时用户明确指定或主要使用的语言；“继续”“下一步”不重置语言，只有用户明确切换时才更新。
标题、正文、提问、图、状态说明和验收摘要统一使用该语言。准入、正式评审、变更、验收等机器侧车
可保留稳定键名和枚举，但交付时必须同时给出同语言的人类可读摘要。结构依次为30秒摘要、任务阅读
地图、业务旅程、模块完整纵切、横切合同和工程/AI索引。
...
证据、缺陷、残余风险和签署结论。结论为接受（`accepted`）、有条件接受（`accepted_with_conditions`）
或拒绝（`rejected`）；有条件接受还要写责任人和完成标准。只有所有强制项有证据且签署允许，需求才能关闭。
...
开发/测试/已部署（`development/test/deployed`）等实现状态只是可选外部里程碑，不能驱动需求状态机。
