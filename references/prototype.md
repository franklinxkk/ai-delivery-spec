...
连贯说明回答“这里做什么、从哪里来、会变成什么、失败怎么办”；涉及写入、状态、权限、口径、跨边界
或恢复时，再在该说明下用默认收起的三段详情补充“前端实现 / 后端处理 / 测试验收”。详细权限、口径、
状态、异常和少量正反例进入“边界与验收”；稳定 ID、字段名、机器枚举与 AC 映射只进入默认收起的
“技术追溯”。机器继续读取同源 handoff。核心字段缺失时显示“待确认/GAP”，不得用空白、角色切换或
通用话术掩盖。
### 2.2 评审工作台（按需加载）
...
| 图 | 必须出现的条件 | 最低信息 |
|---|---|---|
| 核心流程图 | 跨页面/模块/角色，或主链有三步以上依赖/交接 | 起点/终点、责任方、系统边界、关键动作、成功/失败分支和恢复；总览中高亮当前页面/业务浮层 |
| 状态转换图 | 三个以上业务状态，或存在守卫、分支、撤销、回退、超时 | 人类状态标签、触发动作、允许角色、守卫、非法路径、事件/审计和终态；机器枚举进入技术追溯 |
| 数据流/血缘图 | 跨系统、汇聚/转换、AI处理、正反向同步、批量上报或对账 | 权威源、生产者/消费者、业务对象、方向、触发、转换、时效、匹配/去重、失败/重试和对账责任；字段原名进入技术追溯 |

图中节点只使用用户语言；稳定 ID、字段原名和机器枚举的映射保留在默认收起的技术追溯。图后引用详细
合同，不把字段字典、所有异常和验收全文塞入图中。无上述条件的简单单页 CRUD 不画图，只在机器
`diagram_contract` 记录不适用理由，不额外占用人类界面。

产品页面、评审抽屉、图、提示、空态、错误和指标说明都属于人类可见区域，默认只写用户语言。机器
状态、字段、事件或队列对研发有用时，在“技术追溯”中建立本地语言含义与原值的映射，例如“待处理：
`pending`”“同步游标：`lastCursor`”“未知来源：`unknown_source`”；不得把原值裸露成正文标签。
稳定 ID、代码块、公式、`data-*` 锚点和隐藏 handoff 不翻译；品牌或行业通用缩写若可能造成理解歧义，
首次出现补本地语言含义。
