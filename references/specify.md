...
如果用户明确要求 PRD 且交付层级为 L2/L3，实施步骤卡、原型评审抽屉和专项评审说明都不能替代这份统一 PRD。交付前必须运行 PRD 门禁：未知业务决策可以让状态保持 `BLOCKED`，但已知的来源、范围、角色旅程、页面合同、字段/规则/状态、API/事件语义、机器验收和双向追溯不能因“先评审”而省略。专项说明只能标为“补充件”，并引用统一 PRD 的版本/hash 与稳定 ID。
...
3. **角色阅读入口**：第 0 章必须给出角色阅读入口表，声明业务/客户、产品/设计、前端、
   后端/架构、测试/验收和 Coding Agent 各自的先读章节、按稳定 ID 定位的位置和完成标准；
   任何章节合并、拆分或重编号后必须同步该表。
...
交互账本、评审抽屉和 handoff 只能投影同一 PRD 基线，不能反向授权 PRD 中不存在的业务动作、验收
...
人类评审原型只投影页面共识、编号功能点、核心 `STEP-*` 和前端/后端/测试必需差异，不复制 PRD；
Coding Agent 读取结构化 handoff、实施步骤引用、稳定 ID 切片和工程基线。评审完成前，让前端、后端、
测试与 Coding Agent 分别只看交付物复述实现/测试计划；若仍需猜测入口权限、处理/计算口径、状态守卫、
数据权威、事件响应、失败恢复或验收证据，则规格不能声明开发就绪。
